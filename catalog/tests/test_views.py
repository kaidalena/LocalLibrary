...
class AuthorCreateViewTest(TestCase):
    test_user_data = {'login': 'testuser', 'password': '12345'}
    test_super_user_data = {'login': 'superuser', 'password': '12345'}
    new_author_data = {
    ...
    def setUp(self):
        # Создание двух пользователей
        test_user = User.objects.create_user(username=self.test_user_data['login'], password=self.test_user_data['password'])
        test_user.save()

        test_super_user = User.objects.create_user(username=self.test_super_user_data['login'], password=self.test_super_user_data['password'])
        test_super_user.save()
        permission = Permission.objects.get(name='Set book as returned')
        test_super_user.user_permissions.add(permission)
        test_super_user.save()
    ...
    def test_redirect_if_logged_in_but_not_correct_permission(self):
        login = self.client.login(username=self.test_user_data['login'], password=self.test_user_data['password'])
        resp = self.client.get(reverse('author_create'))
    ...
    def test_logged_in_with_permission_create_author(self):
        login = self.client.login(username=self.test_super_user_data['login'], password=self.test_super_user_data['password'])
        resp = self.client.get(reverse('author_create'))
    ...
    def test_form_create_author_date_of_death_initially(self):
        login = self.client.login(username=self.test_super_user_data['login'], password=self.test_super_user_data['password'])
        resp = self.client.get(reverse('author_create'))
        self.assertEqual(resp.status_code, 200)

        self.assertEqual(resp.context['form'].initial['date_of_death'], datetime.date.today().strftime('%d/%m/%Y'))

    def test_redirects_to_all_authors_list_on_success(self):
        login = self.client.login(username=self.test_super_user_data['login'], password=self.test_super_user_data['password'])
        resp = self.client.post(reverse('author_create'), self.new_author_data)
        self.assertEqual(resp.status_code, 302)
